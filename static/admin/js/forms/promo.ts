...
Array('input', 'focusin').forEach((evt) => {
    ...
    PROMO_NAME.addEventListener(evt, function () {
        vc.setValidity(this as HTMLInputElement, this, PURPOSE.PromoName, PURPOSE.PromoName.constraint(this.value));
    });

    PROMO_CODE.addEventListener(evt, function () {
        vc.setValidity(this as HTMLInputElement, this, PURPOSE.PromoCode, PURPOSE.PromoCode.constraint(this.value));
    });

    PROMO_AMOUNT.addEventListener(evt, function () {
        vc.setValidity(this as HTMLInputElement, this, PURPOSE.PromoAmount, PURPOSE.PromoAmount.constraint(this.value));
    });
});
